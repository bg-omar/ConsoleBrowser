...
//  testImplementation("org.jetbrains:bundles.kTest")
}
...
intellij {
  ...
  // Plugin Dependencies. Uses `platformPlugins` property from the gradle.project file.
  //plugins = project("platformPlugins").map { it.split(',').map(String::trim).filter(String::isNotEmpty) }
}
